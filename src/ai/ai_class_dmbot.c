...
void BOT_DMclass_CombatMovement( edict_t *self, usercmd_t *ucmd )
{
		...
		self->enemy && self->client->resp.mos != H_GUNNER)
	{
		...
		if(dist < 150 && infront(self,self->enemy) && self->client->pers.weapon && self->client->pers.weapon->classnameb != WEAPON_MORPHINE)
			ucmd->forwardmove -= 400;
	}
}
...
*/
	...
	else if (randnum == 1  //add random element so player cant keep ducking in and out to avoid being hit
