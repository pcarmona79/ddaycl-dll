...
void SV_CalcGunOffset (edict_t *ent)
{
	...
	ent->client->ps.gunangles[ROLL] = xyspeed * bobfracsin * 0.001;
	...
	ent->client->ps.gunangles[PITCH] = xyspeed * bobfracsin * 0.001;
	...
	/*should do this in md2
	//align weapon md2 better
...
//safe_bprintf (PRINT_HIGH, "2 %i ent->viewheight  %i stance\n", ent->viewheight, ent->stance_view);
	...
	{	// so bobbing only cycles when on ground
		if (xyspeed > 210) // movimiento al correr //hans
			bobmove = 0.1;
		else if (xyspeed > 100)
			bobmove = 0.1;
		else
